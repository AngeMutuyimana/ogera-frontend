...
import { PuzzlePieceIcon, ChevronRightIcon, XMarkIcon } from "@heroicons/react/24/outline";
import {
  useGetMyProblemMetricAttemptHistoryQuery,
  useListPublishedProblemMetricsQuery,
} from "../../services/api/problemMetricApi";

const StudentProblemMetrics: React.FC = () => {
  const [selectedScoreTestId, setSelectedScoreTestId] = React.useState<string | null>(null);
  const { data, isLoading } = useListPublishedProblemMetricsQuery();
  const { data: historyData } = useGetMyProblemMetricAttemptHistoryQuery();
  const tests = data?.data ?? [];
  const history = historyData?.data ?? [];

  const latestByTest = history.reduce<Record<string, (typeof history)[number]>>((acc, item) => {
    if (!acc[item.problem_metric_id]) {
      acc[item.problem_metric_id] = item;
    }
    return acc;
  }, {});

  const selectedTestScores = selectedScoreTestId
    ? history.filter((item) => item.problem_metric_id === selectedScoreTestId)
    : [];
  const selectedTest = selectedScoreTestId
    ? tests.find((t) => t.problem_metric_id === selectedScoreTestId)
    : null;
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Attempt published puzzles to improve the problem-metrics part of your intelligence score.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/50 overflow-hidden shadow-sm">
        {isLoading ? (
          ...
          <p className="p-8 text-center text-gray-500">
            No published puzzle sets yet. Check back when an admin publishes problem metrics.
          </p>
        ) : (
          <ul className="divide-y divide-gray-100 dark:divide-gray-800">
            {tests.map((t) => (
              <li key={t.problem_metric_id}>
                <div className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 hover:bg-gray-50/80 dark:hover:bg-gray-800/40 transition-colors">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5 capitalize">
                      {t.category.replaceAll("_", " ")} · {t.question_count} question{t.question_count === 1 ? "" : "s"}
                    </p>
                    {latestByTest[t.problem_metric_id] && (
                      <div className="mt-2 inline-flex items-center gap-2 rounded-lg border border-[#7F56D9]/30 bg-[#7F56D9]/5 px-3 py-1.5 text-xs">
                        <span className="font-semibold text-[#7F56D9]">Latest score:</span>
                        <span className="text-gray-700 dark:text-gray-200">
                          {latestByTest[t.problem_metric_id].score}/{latestByTest[t.problem_metric_id].max_score}
                        </span>
                        <span className="text-gray-500">
                          ({latestByTest[t.problem_metric_id].percentage}%)
                        </span>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => setSelectedScoreTestId(t.problem_metric_id)}
                      disabled={!history.some((item) => item.problem_metric_id === t.problem_metric_id)}
                    ...
                    <Link
                      to={`/dashboard/problem-metrics/attempt/${t.problem_metric_id}`}
          ...
          <div className="w-full max-w-xl rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-3 mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">{selectedTest?.title || "Selected puzzle set"}</p>
              </div>
              <button
                type="button"
            ...
            {selectedTestScores.length === 0 ? (
              <p className="text-sm text-gray-500">No attempts yet for this puzzle set.</p>
            ) : (
                ...
                {selectedTestScores.map((h) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{new Date(h.taken_at).toLocaleString()}</p>
                    </div>
...
export default StudentProblemMetrics;
