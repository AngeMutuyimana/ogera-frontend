...
const TakeProblemMetric: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data, isLoading } = useGetPublishedProblemMetricQuery(id!, { skip: !id });
  const [submit, { isLoading: submitting }] = useSubmitProblemMetricAttemptMutation();
  ...
  if (!id) {
    return <p className="p-8 text-center text-gray-500">Missing problem metric</p>;
  }

  if (isLoading || !test) {
    return <p className="p-8 text-center text-gray-500">Loading puzzle set...</p>;
  }
      ...
      <div className="max-w-lg mx-auto text-center space-y-6 py-12 px-4">
        ...
        <div className="rounded-2xl border border-[#7F56D9]/30 bg-[#7F56D9]/5 p-8">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-4">
            Your problem-metrics attempts are averaged and contribute 30% of your intelligence score.
          </p>
        </div>
        <button
          type="button"
          onClick={() => navigate("/dashboard/problem-metrics/available")}
          className="text-[#7F56D9] font-medium hover:underline cursor-pointer"
        >
          Back to problem metrics
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-16">
      <Link
        to="/dashboard/problem-metrics/available"
...
export default TakeProblemMetric;
